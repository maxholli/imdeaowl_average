...
//    mean /= e_bits;
//    printf("power %f (%d)\n",mean, e_bits);
//    if (mean>0.15259 && mean<0.1526) {
//	for (int i=0;i<25;i++) {
//		printf("%f\t",fabsf(q->llr[i]));
//	}
//	printf("\n");
//    }
...
uint32_t srslte_pdcch_ue_locations_all(srslte_pdcch_t *q, srslte_dci_location_t *c, uint32_t max_candidates,
	...
	for (l = 3; l >= 0; l--) {
		...
		for (i = 0; i < SRSLTE_MIN(q->nof_cce, 64) / (L); i++) {
			if (l > 0) {
				for (j=0;j<L;j++) {
					if (c[(c[k].ncce + j + SRSLTE_MIN(q->nof_cce, 64) / (8) +  + SRSLTE_MIN(q->nof_cce, 64) / (4) +  + SRSLTE_MIN(q->nof_cce, 64) / (2))].power < PWR_THR) {
						c[k].power = 0;
						break;
					}
				}
			}
			if (c[k].power > PWR_THR) DEBUG("SS Candidate %d: nCCE: %d, L: %d, P: %f\n", k, c[k].ncce, c[k].L, c[k].power);
			k++;
		}
	}
 ...
 *
 */
float srslte_pdcch_decode_msg_check_power(srslte_pdcch_t *q,
                            srslte_dci_msg_t *msg,
                            srslte_dci_location_t *location,
                            srslte_dci_format_t format,
                            uint16_t *crc_rem,
							...
							// IMDEA contribution
{
      ...
      crc_rem           != NULL)
  {
    if (location->ncce * 72 + PDCCH_FORMAT_NOF_BITS(location->L) >
      ...
      float retVec[100];
      for (int i=14; i<=52; i++) {
    	  retVec[i] = dci_decode_and_check_list(q, &q->llr[location->ncce * 72], msg->data, e_bits, i, crc_rem, list);
